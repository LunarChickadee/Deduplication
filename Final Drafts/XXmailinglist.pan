...
global special_user_list,DedupOn, mailing_list_window
permanent username
...
//____uncomment this if you need user control for deduplication_____//
/*
...
There's a need to analize and fix all the logic above because it was set up for the older dedup system. 
and if it's going to work while doing order entry, it will need to be able to handle that logic. 

ask Kate about the different states the file will be in for facilitation vs order entry and
fix the logic above to match. 

Error Noted: Customer# was not found when opening faciltation files, so it was edited out of this version. 
...
message "This function is no longer used, but kept for archive purposes."
