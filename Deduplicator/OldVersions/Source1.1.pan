...
global Dedup_form, Branch_with_most_recent, Field_Nums_Array, most_recent_year,reference_num,
    seed_dict, new_ogs_dict, old_ogs_dict, bulbs_dict, trees_dict, default_blank
...
default_blank=rep(chr(44), arraysize(lineitemarray(SΩ, ","),",")-1)
...
SeedsHistory=?(val(LastSeeds)>0, Seeds_Sales,default_blank)
OGSHistory=?(val(LastOldOGS)>0, OGS_Sales,default_blank)
TreesHistory= ?(val(LastTrees)>0, Trees_Sales,default_blank)
BulbsHistory= ?(val(LastBulbs)>0, Bulbs_Sales,default_blank)
MooseHistory=?(val(LastNewOGS)>0, Moose_Sales,default_blank)
